...
function jsonToHtml(json) {
    ...
    for (object in json) {
        ret += "<li>"+object;
        if( typeof json[object] === "object") ret += jsonToHtml(json[object]);
        else ret += json[object];
        ret += "</li>";
    }
    ret += "</ul>";
    return ret;
...
function jsonToHtml(jsonTree, prefix) {
    ...
    for (object in jsonTree) {
        ...
        $(li).click(function(event) {
        	...
        	selectObject(prefix + '/' + object.toString());
        });
        if (typeof jsonTree[object] === "object") {
        	li.appendChild(jsonToHtml(jsonTree[object], prefix + '/' + object));
        } else {
        	li.firstChild.nodeValue += jsonTree[object];
        }
    }
    return list;
} 
